...
from ..client.thread import Disconnected
...
class Engine(object):
    ...
    def parse(self, acf):
                    ...
                    if anode[0] == "RULE":
                        ...
                        for rnode in anode[4] or []:
                            if rnode[0] in ("UAG", "HAG"):
                                ...
                                warnings.warn(
                                    "Invalid RULE condition AST: %s" % (rnode,)
                                )
        ...
        if self._ctxt is not None:
            [
                _log.debug("subscribing to %s for %s", pv, grps)
                for pv, grps in invars.items()
            ]
            self._subscriptions = {
                pv: self._ctxt.monitor(
                    pv, partial(self._var_update, grps), notify_disconnect=True
                )
                for pv, grps in invars.items()
            }
        else:
    ...
    def create(self, channel, group, user, host, level, roles=[]):
        ...
        perm = 0
        _log.debug(
            "(re)create %s, %s, %s, %s, %s", channel.name, group, user, host, level
        )
        ...
        with self._lock:
            ...
            try:
                for mask, asl, trap, conds in rules:
                    ...
                    for cond in conds:
                        if cond[0] == "UAG":
                            ...
                            try:
                                accept = (
                                    float(eval(cond[2], {}, inputs) or 0.0) >= 0.5
                                )  # horray for legacy... I mean compatibility
                            except Exception:
                                # this could be any of a number of exceptions
                                # which all add up to the same.  Invalid expression
                                accept = False
                                _log.exception(
                                    "Error evaluating: %s with %s",
                                    cond[1],
                                    [(k, v, type(v)) for k, v in inputs.items()],
                                )
                            else:
                                _log.debug(
                                    "Evaluate %s with %s -> %s",
                ...
                _log.exception(
                    "Error while calculating ASG for %s, %s, %s, %s, %s",
                    channel,
                    group,
                    user,
                    host,
            ...
            channel.access(
                put=bool(put), rpc=bool(rpc), uncached=bool(uncached), audit=trapit
            )

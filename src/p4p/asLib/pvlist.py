...
def _sub_add(expr, ngroups, adjust=0):
    ...
    def check(M):
        ...
        if grp < 1 or grp > ngroups:
            raise ValueError(
                "Out of range substitution %d.  Must be in [1, %d]" % (grp, ngroups)
            )
...
class PVList(object):
    ...
    def __init__(self, pvl):
            ...
            try:
                ...
                if M:
                    if M.groups() == ("DENY", "ALLOW"):
                        ...
                        _log.warn(
                            'Ignoring "EVALUATION ORDER DENY, ALLOW".  Only ALLOW, DENY is implemented.'
                        )
                    elif M.groups() == ("ALLOW", "DENY"):
                        allowfirst = (  # noqa: F841
                            False  # deny rules take precedence (default)
                        )
                    else:
        ...
        self._deny_from = {
            addr: _re_join(list(exprs) + deny_all, "?:")
            for addr, exprs in deny_from.items()
        }

...
def getargs():
    ...
    P.add_argument(
        ...
        metavar="name=type",
        nargs="+",
        help="PV definitions.  types: %s" % (", ".join(defs.keys())),
    )
...
def main(args):
    ...
    if args.file and os.path.exists(args.file):
        ...
        if persist["version"] != 1:
            warnings.warn(
                "Unknown persist version %s.  Attempting to load" % persist["version"]
            )
        persist = persist["pvs"]
    ...
    try:
        ...
        if args.file:
            OF.write(
                json.dumps(OrderedDict([("version", 1), ("pvs", persist)]), indent=2)
            )
            OF.flush()
            OF.close()

...
from . import _gw, listRefs, set_debug
from .asLib import Engine
from .asLib.lex import ACFError
from .asLib.pvlist import PVList
from .client.thread import Context
from .nt import NTScalar, NTTable, Type
from .server import Server, StaticProvider, removeProvider
from .server.thread import RemoteError, SharedPV
...
class RefAdapter(object):
    def __init__(self):
        self.type = NTTable.buildType(
            [("type", "as"), ("count", "aI"), ("delta", "ai")]
        )
        self.prev = {}
...
class GWStats(object):
    ...
    def __init__(self, statsdb=None):
        ...
        self._pvs["StatsTime"] = self.statsTime = SharedPV(
            nt=NTScalar("d"), initial=0.0
        )

        # faulthandler.dump_traceback() added w/ py 3.3
        # however, file= arg. added w/ 3.5
        if sys.version_info >= (3, 5):
            self._pvs["threads"] = stackstrace = SharedPV(
                nt=NTScalar("s"), initial="RPC only"
            )
    ...
    def update_stats(self, norm):
        ...
        with closing(sqlite3.connect(self.statsdb)) as C, C:
            ...
            for handler in self.handlers:
                C.executemany(
                    "INSERT INTO us VALUES (?,?,?,?,?,?)", handler.provider.report(norm)
                )

            for server in self.servers:
                C.executemany(
                    "INSERT INTO ds VALUES (?,?,?,?,?,?,?,?)",
                    _gw.Server_report(server, norm),
                )
            ...
            self.tbl_usbypvtx.post(
                C.execute(
                    "SELECT usname, tx as rate FROM usbyname ORDER BY rate DESC LIMIT 10"
                )
            )
            self.tbl_usbypvrx.post(
                C.execute(
                    "SELECT usname, rx as rate FROM usbyname ORDER BY rate DESC LIMIT 10"
                )
            )

            self.tbl_usbyhosttx.post(
                C.execute(
                    "SELECT peer, tx as rate FROM usbypeer ORDER BY rate DESC LIMIT 10"
                )
            )
            self.tbl_usbyhostrx.post(
                C.execute(
                    "SELECT peer, rx as rate FROM usbypeer ORDER BY rate DESC LIMIT 10"
                )
            )

            self.tbl_dsbypvtx.post(
                C.execute(
                    "SELECT usname, tx as rate FROM dsbyname ORDER BY rate DESC LIMIT 10"
                )
            )
            self.tbl_dsbypvrx.post(
                C.execute(
                    "SELECT usname, rx as rate FROM dsbyname ORDER BY rate DESC LIMIT 10"
                )
            )

            self.tbl_dsbyhosttx.post(
                C.execute(
                    "SELECT account, peer, tx as rate FROM dsbypeer ORDER BY rate DESC LIMIT 10"
                )
            )
            self.tbl_dsbyhostrx.post(
                C.execute(
                    "SELECT account, peer, rx as rate FROM dsbypeer ORDER BY rate DESC LIMIT 10"
                )
            )

            self.clientsPV.post(
                [row[0] for row in C.execute("SELECT DISTINCT peer FROM us")]
            )
        ...
        cachepvs = list(
            reduce(
                set.__or__,
                [handler.provider.cachePeek() for handler in self.handlers],
                set(),
            )
        )
        cachepvs.sort()
...
class GWHandler(object):
    ...
    def makeChannel(self, op):
        ...
        if (
            not usname
            or self.provider.testChannel(usname.encode()) != self.provider.Claim
        ):
            return None
...
def getargs():
    ...
    P.add_argument(
        "--no-ban-local", action="store_true", help="Legacy option.  Ignored"
    )
    P.add_argument(
        "-v",
        "--verbose",
    ...
    P.add_argument(
        "--logging", help="Use logging config from file (JSON in dictConfig format)"
    )
    P.add_argument(
        "--debug",
        action="store_true",
...
class App(object):
    def __init__(self, args):
        ...
        try:
            ...
            if jver not in (1, 2):
                _log.error(
                    "Warning: config file version %d not in range [1, 2]\n" % jver
                )
        ...
        for jcli in jconf["clients"]:
            ...
            client_conf = {
                ...
                "EPICS_PVA_AUTO_ADDR_LIST": {True: "YES", False: "NO"}[
                    jcli.get("autoaddrlist", True)
                ],
            }
            ...
            if args.test_config:
                ...
                clients[name] = Context(jcli.get("provider", "pva"), client_conf)
        ...
        for jsrv in jconf["servers"]:
            ...
            if jver == 1:
                ...
                if isinstance(iface, list):
                    _log.warning(
                        "Server interface list should specify JSON scheme version 2"
                    )
                else:
                    # be forgiving
                    iface = [iface]
            ...
            if len(jsrv.get("addrlist", "")) > 0 and len(iface) > 1:
                _log.warning(
                    "Server entries for more than one interface must not specify addrlist."
                )
                _log.warning(
                    "Each server interface will attempt to send beacons to all destinations"
                )
                jsrv.pop("addrlist")
        ...
        for jsrv in jconf["servers"]:
            ...
            server_conf = {
                ...
                "EPICS_PVAS_AUTO_BEACON_ADDR_LIST": {True: "YES", False: "NO"}[
                    jsrv.get("autoaddrlist", True)
                ],
            ...
            if aclient is None:
                if len(jsrv["clients"]) > 1 and "access" in jsrv:
                    _log.warning(
                        "Multiple clients and ACF is ambigious.  Add key 'acf_client' to disambiguate"
                    )
            ...
            statusp = StaticProvider("gwsts." + name)
            providers = [statusp]
            self.__lifesupport += [statusp]

            try:
                for client in jsrv["clients"]:
                    pname = "gws.%s.%s" % (name, client)

                    client = clients[client]

                    handler = GWHandler(
                        access, pvlist, readOnly=jconf.get("readOnly", False)
                    )
                    handler.getholdoff = jsrv.get("getholdoff")

                    if not args.test_config:
                        handler.provider = _gw.Provider(
                            pname, client, handler
                        )  # implied installProvider()
                ...
                if "statusprefix" in jsrv:
                    ...
                    handler.asTestPV = SharedPV(
                        nt=NTScalar("s"), initial="Only RPC supported."
                    )
                    handler.asTestPV.rpc(
                        handler.asTest
                    )  # TODO this is a deceptive way to assign
                    statusp.add(jsrv["statusprefix"] + "asTest", handler.asTestPV)

                    handler.asDebugPV = SharedPV(
                        nt=NTScalar("s"), initial="Only RPC supported."
                    )
                    handler.asDebugPV.rpc(
                        handler.asDebug
                    )  # TODO this is a deceptive way to assign
                ...
                try:
                    server = Server(providers=providers, conf=server_conf, useenv=False)
                except RuntimeError:
                    _log.exception(
                        "Unable to create server %s", pprint.pformat(server_conf)
                    )
                    sys.exit(1)
...
def main(args=None):
    ...
    if catfile is not None:
        ...
        modroot = os.path.dirname(
            os.path.dirname(__file__)
        )  # directory containing p4p/gw.py

...
"""
        ...
        self.assertEqual(
            pvl.compute(b"BEAM:stuff", "127.0.0.1"), ("BEAM:stuff", "RWINSTRMCC", 1)
        )
        self.assertEqual(
            pvl.compute(b"BEAM:L1:Energy", "127.0.0.1"),
            ("BEAM:L1:Energy", "DEFAULT", 0),
        )

        self.assertEqual(
            pvl.compute(b"OTHER:stuff", "127.0.0.1"), ("OTHER:stuff", "DEFAULT", 0)
        )

        self.assertEqual(
            pvl.compute(b"OTRS:DMP1:695:Image:X", "127.0.0.1"), (None, None, None)
        )

        self.assertEqual(
            pvl.compute(b"PATT:SYS0:1:MPSBURSTCTRLX", "127.0.0.1"),
            ("PATT:SYS0:1:MPSBURSTCTRLX", "CANWRITE", 0),
        )
        self.assertEqual(
            pvl.compute(b"PATT:SYS0:1:MPSBURSTCTRLX", "1.2.3.4"), (None, None, None)
        )
        self.assertEqual(
            pvl.compute(b"PATT:SYS0:1:MPSBURSTCTRLX", "4.3.2.1"), (None, None, None)
        )

        self.assertEqual(
            pvl.compute(b"Xsomething", "127.0.0.1"), ("Ysomething", "CANWRITE", 0)
        )
        self.assertEqual(
            pvl.compute(b"a:one:b:two", "127.0.0.1"), ("A:two:B:one", "DEFAULT", 0)
        )
        ...
        self.assertEqual(
            ...
            [
                ...
                (
                    "HAG",
                    "GWSTATS",
                    ["lcls-daemon3", "other.host", "strange", "invalid.host.name."],
                ),
                ("ASG", "SIMPLE", [("RULE", 1, "READ", False, None)]),
                (
                    "ASG",
                    "NOTSIMPLE",
                    [
...
class TestACL(unittest.TestCase):
    ...
    def test_default(self):
        ...
        self.assertDictEqual(
            ch.perm, {"put": True, "rpc": True, "uncached": True, "audit": False}
        )
...
""")
        ...
        for args, perm in [
            (
                ("DEFAULT", "someone", "somewhere", 0),
                {"put": False, "rpc": False, "uncached": False, "audit": False},
            ),
            (
                ("DEFAULT", "root", "1.2.3.4", 0),
                {"put": True, "rpc": True, "uncached": False, "audit": False},
            ),
            (
                ("DEFAULT", "someone", "1.2.3.4", 0, ["admin"]),
                {"put": True, "rpc": True, "uncached": False, "audit": False},
            ),
        ]:
            try:
...
""")
        ...
        for args, perm in [
            ...
            (
                ("CANWRITE", "someone", "somewhere", 0),
                {"put": True, "rpc": True, "uncached": False, "audit": True},
            ),
            (
                ("CANWRITE", "root", "1.2.3.44", 0),
                {"put": True, "rpc": True, "uncached": False, "audit": True},
            ),
            (
                ("AMOWRITE", "someone", "somewhere", 0),
                {"put": False, "rpc": False, "uncached": False, "audit": False},
            ),
            (
                ("AMOWRITE", "someone", "1.2.3.44", 0),
                {"put": False, "rpc": False, "uncached": False, "audit": False},
            ),
            (
                ("AMOWRITE", "root", "somewhere", 0),
                {"put": False, "rpc": False, "uncached": False, "audit": False},
            ),
            (
                ("AMOWRITE", "root", "1.2.3.44", 0),
                {"put": True, "rpc": True, "uncached": False, "audit": True},
            ),
        ]:
            try:
...
""",
        ...
        db = []
        for args in [
            ("OPERATOR", "joe", "1.2.3.10", 0),  # always allowed MCR

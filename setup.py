...
from setuptools_dso import Extension, cythonize, setup
...
exts = cythonize(
    [
        Extension(
            ...
            dsos=["pvxslibs.lib.pvxs", "epicscorelibs.lib.Com"],
            libraries=get_config_var("LDADD"),
        ),
        Extension(
            name="p4p._gw",
            sources=["src/p4p/_gw.pyx", "src/pvxs_gw.cpp", "src/pvxs_odometer.cpp"],
            include_dirs=get_numpy_include_dirs()
            + [
                epicscorelibs.path.include_path,
                pvxslibs.path.include_path,
